...
Mac 和 Linux 桌面环境直接可用（自动探测 Chrome / Chromium 路径，Mac 上会自动把 Chrome 窗口最小化）。默认只监听 `127.0.0.1:8271`；想让同一局域网的手机连，设 `BROWSER_RELAY_HOST=0.0.0.0`——但注意这是裸 HTTP，密码会明文过网，**只能在可信局域网这么干**，公网必须走下面的隧道方案。
...
- **登录**：手机端把密码原样发给服务器（请求头 `X-Relay-Auth`），服务器验证通过后下发另一个派生值作为 **HttpOnly** cookie（30 天）。早期版本是在客户端先做一次 sha256 再发，后来改回明文，原因有两条：一是 `crypto.subtle` 只在"安全上下文"（HTTPS 或 localhost）里可用，而这个服务的典型用法恰恰是`http://局域网IP:8271`，在那里 `crypto.subtle` 是 undefined，登录直接是坏的；二是客户端哈希本来也**不提供**任何抗嗅探能力——服务器比对的就是那个哈希，谁在网络上截到它谁就能登录，它本身就是密码。**真正的传输保护只能靠 HTTPS/隧道**，见上面的公网方案；在裸 HTTP 上，明文和哈希一样不安全。页面里的 JS 读不到这个 cookie；恶意网页就算把标签页标题设成一段脚本，也只会被当纯文本渲染。改密码 = 所有已登录设备立刻失效。
- **三条铁律**：
